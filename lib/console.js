...
const init = () => {
    ...
    switch (process.env.POWN_CONSOLE_OUTPUT_FORMAT || defaultOutputFormat) {
        ...
        case 'json':
            Object.keys(consoleDef).forEach((type) => {
                console[type] = ((fn) => (...args) => {
                    try {
                        fn(JSON.stringify({ type, data: (args.length > 1 ? args : args[0]) }, replacer))
                    }
                    catch (e) {
                        console.error(e)
                    }
    ...
    if (process.env.POWN_DEBUG || process.env.POWN_DEBUG_L || process.env.POWN_DEBUG_XL || process.env.POWN_DEBUG_XXL) {
        if (!process.env.POWN_DEBUG_L) {
            console.debug = ((fn) => (...args) => {
                fn(...args.map(data => data && Buffer.isBuffer(data) ? data.slice(0, 64) : data))
            })(console.debug)
        }
        else
        if (!process.env.POWN_DEBUG_XL) {
            console.debug = ((fn) => (...args) => {
        ...
        if (!process.env.POWN_DEBUG_XXL) {
            console.debug = ((fn) => (...args) => {
                fn(...args.map(data => data && Buffer.isBuffer(data) ? data : data))
            })(console.debug)
        }
    }
    else {
    ...
    switch (process.env.POWN_CONSOLE_TABLE_OUTPUT_FORMAT || process.env.POWN_CONSOLE_OUTPUT_FORMAT || defaultOutputFormat) {
        case 'node':
            console.table = (data) => {
        ...
        case 'text':
            console.table = (data, properties = null, options = {}) => {
                ...
                if (!properties && data.length > MAX_PROPERTYLESS_ROWS) {
                    console.warn('attempting to print a table with too many rows and anundefined number of properties')
                }

                let head = properties || Array.from(new Set(data.reduce((a, v) => {
                    return a.concat(Object.keys(v))
                }, [])))

                if (head.length > MAX_HEAD_FIELDS) {
                    console.warn(`table head is ${MAX_HEAD_FIELDS - head.length} fields larger then the maximum supported`)
                }
                ...
                if (span) {
                    data.forEach((entry) => {
                        entry = head.map((n) => entry[n] ? (entry[n].toString ? entry[n].toString() : '-') : '')
        ...
        default:
            console.table = (data, properties = null, options = {}) => {
                ...
                if (!properties && data.length > MAX_PROPERTYLESS_ROWS) {
                    console.warn('attempting to print a table with too many rows and an undefined number of properties')
                }
