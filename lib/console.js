...
const init = () => {
    const DEFAULT_OUTPUT_FORMAT = process.stdout.isTTY ? 'pown' : 'text'

    switch (process.env.POWN_CONSOLE_OUTPUT_FORMAT || DEFAULT_OUTPUT_FORMAT) {
        case 'node':
    ...
    switch (process.env.POWN_CONSOLE_TABLE_OUTPUT_FORMAT || process.env.POWN_CONSOLE_OUTPUT_FORMAT || DEFAULT_OUTPUT_FORMAT) {
        case 'node':
            console.table = (data) => {
        ...
        case 'text':
            console.table = (data, properties = null, options = {}) => {
                ...
                if (!properties && data.length > MAX_PROPERTYLESS_ROWS) {
                    console.warn('attempting to print table with too many rows and undefined number of properties')
                }
