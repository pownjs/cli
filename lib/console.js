...
const init = () => {
    switch (process.env.POWN_CONSOLE_OUTPUT_FORMAT || (process.stdout.isTTY ? 'pown' : 'json')) {
        case 'node':
    ...
    switch (process.env.POWN_CONSOLE_TABLE_OUTPUT_FORMAT || process.env.POWN_CONSOLE_OUTPUT_FORMAT || (process.stdout.isTTY ? 'pown' : 'json')) {
        case 'node':
            console.table = (data) => {
        ...
        case 'json':
            console.table = (data) => outConsole.log(JSON.stringify({ type: 'table', data: data }))
